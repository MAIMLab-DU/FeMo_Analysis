    ...
    def _get_ml_detection_map(self,
        ...
        if num_labels:  # When there is a detection by the sensor system
            for k in range(1, num_labels + 1):
                ...
                if overlap:
                    # This is a TPD
                    if matching_index_tpd < len(overall_tpd_pred) and overall_tpd_pred[matching_index_tpd] == 1:
                        segmented_sensor_data_ml[label_start:label_end] = 1
                    matching_index_tpd += 1
                    tpd_indices.append(k)
                else:
                    # This is an FPD
                    if matching_index_fpd < len(overall_fpd_pred) and overall_fpd_pred[matching_index_fpd] == 1:
                        segmented_sensor_data_ml[label_start:label_end] = 1
