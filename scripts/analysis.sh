...
python "$(convert_path "$SCRIPT_DIR/repack.py")" --model "$(find "$run_dir/model" -name 'model.*' -print -quit)" --classifier "$run_dir/model/classifier.joblib" --pipeline "$run_dir/pipeline/pipeline.joblib" --processor "$run_dir/processor/processor.tar.gz" --metrics "$run_dir/metrics/metrics.joblib" --work-dir "$run_dir"
