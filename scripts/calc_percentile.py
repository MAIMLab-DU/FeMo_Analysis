...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="Path to directory containing .dat and .csv files",
    )
    parser.add_argument(
        "--config-path",
        type=str,
        default=os.path.join(BASE_DIR, "..", "configs/dataset-cfg.yaml"),
...
def main(args):
    ...
    dataset = FeMoDataset(args.data_dir, data_manifest, False, dataset_cfg.get("data_pipeline"))

    sensor_sensation_peaks = {key: [] for key in dataset.pipeline.get_stage("load").sensors}
    ...
    for item in tqdm(data_manifest["items"], desc="Calculating segment peaks", unit="file"):
        ...
        data_success = dataset._download_from_s3(filename=data_filename, bucket=bucket, key=data_file_key)
        if not data_success:
            dataset.logger.warning(f"Failed to download {data_filename} from {bucket = }, {data_file_key =}")
            continue

        loaded_data = dataset.pipeline.get_stage("load").transform(data_filename)
        preprocessed_data = dataset.pipeline.get_stage("preprocess").transform(loaded_data)

        for sensor in dataset.pipeline.get_stage("load").sensors:
            sensor_data = preprocessed_data[sensor]
            sensation_data = preprocessed_data["sensation_data"]
            peaks = calculate_segment_peaks(sensor_data, sensation_data, dataset.pipeline.self.get_stage("segment"))
            sensor_sensation_peaks[sensor].extend(peaks)

    data_manifest["percentiles"] = {"nth_percent": args.nth_percentile}
